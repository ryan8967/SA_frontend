<template>
  <div class="card-block">
    ...
    <div
      v-if="wordData && wordData.length && testCompleted"
...
export default {
  ...
  methods: {
    ...
    nextQuestion() {
      ...
      this.currentChoices = [correctAnswer, ...wrongAnswers].sort(
        () => 0.5 - Math.random()
      );
    ...
    async handleIncorrectWord(word) {
      try {
    ...
    async submitAnswer() {
      if (this.selectedAnswer === this.currentQuestion.word) {
        ...
        await this.handleIncorrectWord(this.currentQuestion.word);
      }
    ...
    async getCardFromGPT() {
      ...
      let existingWords = []; // Initialize an empty array
      try {
        const response = await fetch("http://localhost:8080/api/words/get", {
          method: "GET",
        });
        
        if (response.ok) {
          existingWords = await response.json(); // Assume API returns an array of words
        } else {
      ...
      const existingWordsStr = existingWords.join(", "); // Convert array to string for the prompt
      ...
      const prompt = `You are an English teacher to help the user understand an English word.
          Here is the given topic: "${this.inputWord}". Generate ${this.wordCount} words serving a-kind-of relationship to this topic. 
          Avoid generating these words: ${existingWordsStr}. Please answer with a JSON format string as follows:
